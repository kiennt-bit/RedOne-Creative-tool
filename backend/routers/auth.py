...
async def callback(request: Request, code: str = "", state: str = "", error: str = ""):
    ...
    try:
        ...
        if hub_client.is_enabled():
            ...
            # Pull the team's shared account → push the shared Shakker token
            # into the local account so this member gens zero-login.
            await hub_client.sync_shared_credentials()
    except Exception:
