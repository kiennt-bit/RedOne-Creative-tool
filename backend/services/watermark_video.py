...
log = logging.getLogger("redone.watermark_video")
...
# ── Fast path: ffmpeg delogo + maskedmerge ──────────

async def _run_ffmpeg_delogo(
    ffmpeg: str,
    input_video: Path,
    output_video: Path,
    mask_src: Path,
    ...
    """Fast watermark removal: a SINGLE ffmpeg pass that `delogo`s the logo box
    and `maskedmerge`s only the alpha-glyph pixels back over the original.
    Shape-following (only the "Veo" letters change), no Python/torch needed.

    Encoder is libx264 (portable) for broad compatibility (not NVENC-locked).
    """
    import re
    ...
    mask_filter = (
        ...
        f"lut=c0='if(gt(val,15),255,0)',format=gray8"
    )
    filter_complex = (
        f"[1:v]{mask_filter}[mask];"
        f"[0:v]split[orig][forfix];"
        f"[forfix]delogo=x={logo_x}:y={logo_y}:w={logo_w}:h={logo_h}:show=0[fixed];"
        f"[orig][fixed][mask]maskedmerge[out]"
    )
    ...
    await emit("Đang xóa watermark…", 1)
    proc = await asyncio.create_subprocess_exec(
        *args,
        stdout=asyncio.subprocess.DEVNULL,
    ...
    while True:
        ...
        m = re.search(r"time=(\d+):(\d+):(\d+)", s)
        if m and total_secs > 0:
            cur = int(m.group(1)) * 3600 + int(m.group(2)) * 60 + int(m.group(3))
            pct = min(99, max(1, round(cur / total_secs * 100)))
            await emit(f"Đang xóa watermark… {pct}%", pct)

    rc = await proc.wait()
    if rc != 0:
        raise RuntimeError(
            "Xóa watermark (ffmpeg) thất bại: "
            + tail.decode("utf-8", errors="replace")[-400:]
        )
    await emit("Hoàn tất", 100)
    return output_video
...
async def remove_watermark_from_video(
    ...
    # FAST path (default) — single ffmpeg delogo+maskedmerge pass:
    # one ffmpeg delogo+maskedmerge pass. Used for auto/ffmpeg/fast/opencv.
    # AI methods (lama/sttn/propainter) fall through to the subprocess path.
    _mask_for_fast = mask_path or get_default_veo_mask()
    if not Path(_mask_for_fast).exists():
        raise RuntimeError(f"Mask file không tồn tại: {_mask_for_fast}")
    # Engine choice: LaMa AI gives the clean,
    # high-quality result (its "xịn" output); the fast ffmpeg delogo+maskedmerge
    # is only the no-Python fallback (leaves a faint trace).
    #   • ffmpeg/fast/delogo  → always the fast delogo path
    #   • lama/sttn/propainter → the AI subprocess path (below)
    #   • auto/opencv          → LaMa when its deps (Python+torch+model) are
    #                            ready; otherwise fall back to fast delogo
    if method in ("ffmpeg", "fast", "delogo"):
        return await _run_ffmpeg_delogo(
            ffmpeg, input_video, output_video, Path(_mask_for_fast), _emit
        )
    if method in ("auto", "opencv"):
        ...
        if st.get("lama_ok"):
            ...
            await _emit("LaMa chưa sẵn sàng — dùng xóa nhanh (delogo)", 1)
            return await _run_ffmpeg_delogo(
                ffmpeg, input_video, output_video, Path(_mask_for_fast), _emit
            )
    ...
    work_dir = Path(tempfile.mkdtemp(prefix=f"redone_wm_{os.getpid()}_"))
    ...
    try:
        ...
        # 2. (No mask pre-processing.) lama_inpaint.py takes the RAW mask
        #    (RGBA glyph, 3840×2160) directly and crops to the watermark
        #    region itself via _find_crop_region.
        #    Pre-scaling the mask to frame size would trigger pre_cropped mode
        #    = LaMa over the WHOLE frame (same result but much slower).
        ...
        if not use_subprocess:
            ...
            args = [
                ...
                str(frames_in), str(mask_src), str(frames_out),
