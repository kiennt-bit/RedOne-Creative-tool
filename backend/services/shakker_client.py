...
class ShakkerClient:
    ...
    async def upload_reference_image(self, file_bytes: bytes,
        ...
        Requires `oss2` (lazy-imported so t2i-only installs don't need it).
        """
        try:
            import oss2  # type: ignore
        except ImportError:
            raise ShakkerError(
                "Thiếu thư viện oss2 để upload ảnh tham chiếu. "
                "Cài: pip install oss2",
        ...
        sts = await self._get("/gateway/common-server-api/common-service/api/sts/v2/0")
        access_key_id = sts.get("accessKeyId")
        access_key_secret = sts.get("accessKeySecret")
        security_token = sts.get("securityToken")
        if not (access_key_id and access_key_secret and security_token):
            raise ShakkerError("STS không trả đủ credentials", raw=str(sts)[:200])

        # 2. Compute object key = img/{userUuid}/{sha256}.{ext}
        file_hash = hashlib.sha256(file_bytes).hexdigest()
        if ext.lower() in ("jpg", "jpeg"):
            ext = "jpeg"
        object_key = f"img/{self.user_uuid}/{file_hash}.{ext}"

        # 3. Upload via OSS (sync SDK → run in thread)
        def _do_upload() -> str:
            auth = oss2.StsAuth(access_key_id, access_key_secret, security_token)
            bucket = oss2.Bucket(
                auth,
                "https://oss-us-east-1.aliyuncs.com",
                "models-online-persist-us",
            )
            bucket.put_object(object_key, file_bytes)
            return f"https://models-online-persist.shakker.cloud/{object_key}"

        try:
            cdn_url = await asyncio.to_thread(_do_upload)
        except Exception as e:
            raise ShakkerError(f"Upload ảnh tham chiếu thất bại: {e}", raw=str(e))
    ...
    async def submit_generate(self, payload: dict) -> str:
        ...
        for attempt in range(6):
            try:
                data = await self._post("/gateway/sd-api/gen/tool/shake", payload)
