...
class TeamCredentials(Base):
    """Shared account a whole team uses (the lead's Google Flow + Shakker
    account). Members auto-login with these without ever seeing them. The
    secret fields are stored ENCRYPTED (Fernet, see encryption.py); only the
    email is plaintext (it's not the sensitive part). One row per team.

    Settable by the team's lead or an admin; readable (decrypted) by members
    of the team for auto-login."""

    __tablename__ = "team_credentials"

    team_id = Column(Integer, ForeignKey("teams.id"), primary_key=True)
    google_email = Column(String(255), default="")
    google_password_enc = Column(Text, nullable=True)
    shakker_token_enc = Column(Text, nullable=True)
    shakker_webid_enc = Column(Text, nullable=True)
